...
export const PageCard = ({ href, children, textColor = 'indigo', toTitle }: PageCardProps) => {
  ...
  return (
    <div className={`p-8 rounded-lg shadow-sm border`}>
      <h2 className={`uppercase font-medium mb-3 ${colorClass}`}>{toTitle}</h2>
      <div>{children}</div>
      <Link href={href}>
        <span
          className={`flex items-center gap-1 transition-colors duration-200 ${colorClass} text-sm font-medium`}
        >
          Go To {toTitle}
        </span>
      </Link>
